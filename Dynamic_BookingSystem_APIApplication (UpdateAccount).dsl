workspace "Booking Care"  "This is an example workspace to illustrate the key features of Structurizr, via the DSL, based around a fictional online banking system." {
    ...
    enterprise "Booking Care" {
      ...
      bookingCareSystem = softwareSystem "Booking System" "Allow all customer to view information about their booking accounts, and booking" {
        ...
        apiApplication = container "API Application" "Provides Booking care functionality via a JSON/HTTPS API." "Node JS Express MVC"{
          ...
          showAccount = component "Show Account Doctor" "Display all account doctoc"
        }
  ...
  //Update Doctor Information
    ...
    updateInfoDoctor -> securityComponent
    securityComponent -> database 
    securityComponent -> email
    database -> updateInfoDoctor
    updateInfoDoctor -> singlePageApplication
    
    //Update Account
    admin -> singlePageApplication
    singlePageApplication -> updateAccount
    updateAccount -> securityComponent
    securityComponent -> database 
    securityComponent -> email
    database -> updateAccount
    updateAccount -> singlePageApplication
    
    // Add Accout
    admin -> singlePageApplication
    singlePageApplication -> addAccount 
    addAccount  -> securityComponent 
    securityComponent -> database 
    database -> showAccount 
    showAccount -> singlePageApplication
...
//
  ...
  views {
    ...
    dynamic apiApplication "UpdateDoctorInformation" "Update Doctor Information valid with required" {
      doctor -> singlePageApplication "Uses"
      singlePageApplication -> updateInfoDoctor "Submits credentials to"
      updateInfoDoctor -> securityComponent "Validates credentials using"
      securityComponent -> database "Update Doctor Information"
      securityComponent -> email "Information has been updated"
      database -> updateInfoDoctor "Get information from database to show"
      updateInfoDoctor -> singlePageApplication "Show the update information"
      autoLayout
     }
     
     dynamic apiApplication "UpdateAccount" "Update Account valid with required"{
      admin -> singlePageApplication "Uses"
      singlePageApplication -> updateAccount "Submits credentials to"
      updateAccount -> securityComponent "Validates credentials using"
      securityComponent -> database "Update Account"
      securityComponent -> email "Account has been updated"
      database -> updateAccount "Get information from database to show"
      updateAccount -> singlePageApplication "Show the update account"

      autoLayout
    }

    dynamic apiApplication "AddAcounttt" "Add acount new doctor"{
      doctor -> singlePageApplication "Uses"
      singlePageApplication -> addAccount "Add a new doctor"
      addAccount  -> securityComponent "Validates credentials using"
      securityComponent -> database "Add information doctor in database"
      database -> showAccount "Get information from database to show"
      showAccount -> singlePageApplication "Display new  account  doctor"
